...
{
    id: 'reset_default',
    className: 'fas fa fa-eraser icon-redo',
    command: 'html-reset-default',
    attributes: {
...
},
{
    ...
    command: function (editor, sender) {
        ...
        const getPage = pageManager.get(allPages[0].id)
        localStorage.setItem('template-name', allPages[0].id)
...
{
    id: 'template staking-2',
    className: 'template',
    command: function (editor, sender) {
        const pageManager = editor.Pages;
        const allPages = pageManager.getAll()
        const getPage = pageManager.get(allPages[1].id)
        localStorage.setItem('template-name', allPages[1].id)
        const el = document.querySelector('[title="staking"]');
        const buttons = document.querySelectorAll('.template');
    ...
    command: (editor, sender) => {
        ...
        if (onclick=window.confirm('Are you sure to clean the canvas?')) {
            ...
            setTimeout(function () {
                ...
                localStorage.setItem('gjs-script', '');
    ...
    command: (editor, sender) => {
        ...
        const form = `
            ...
            <form id="deploy-form" class="gram-form">
                ...
                <h4>Netlify</h4>
                <div class="form-group">
                    <label>Get Token <span class="required">*</span> <a target="_blank" href="https://app.netlify.com/user/applications#personal-access-tokens"><span class="fa fa-question"></span></a></label>
...
},
{
    ...
    command: (editor, sender) => {
        ...
        const contract = localStorage.getItem('autodapp_contact')
        const stakingContract = localStorage.getItem('autodapp_contact_staking')
        const abi = localStorage.getItem('autodapp_abi') 
        const stakingAbi = localStorage.getItem('autodapp_staking_abi')

        if (!contract || !stakingContract || !abi || !stakingAbi) {
             if (!window.confirm('you do not have all the fields of the contract filled in, are you sure you want to save without a contract?')) {
        ...
        exportForm.addEventListener('submit', async (e) => {
            ...
            const data = await getAllPages(editor)
            exportZip({...data, title, description});
